...
<head>
  ...
  <title>عزّنا بفخرِنا — حكايات بطولية من الوطن</title>
  <meta name="description" content="ثماني قصص بطولية حقيقية من تاريخ المملكة العربية السعودية وشخصيات وطنية، مع نصّين موثّقين ومصادر معتمدة."/>
  <style>
    :root{
      --green:#0a8a43; --green-dark:#0a6a36; --edge:#0e3b22;
      --bg:#041b10; --card:#082816; --text:#f5fff9; --muted:#cfe9dc; --gold:#ffd166; --white:#ffffff;
    }
    *{box-sizing:border-box}
    html,body{margin:0;padding:0;background:
      radial-gradient(1200px 700px at 20% -10%, #06331b 0%, transparent 60%),
      linear-gradient(180deg, #02160c 0%, #031d11 60%, #02160c 100%);
      color:var(--text);font-family:system-ui,-apple-system,Segoe UI,Roboto,"Noto Naskh Arabic","Amiri",Tahoma,Arial,sans-serif;line-height:1.9}
    a{color:#9ff7c9;text-decoration:none}
    a:hover{text-decoration:underline}
    .container{max-width:1200px;margin:auto;padding:24px}
    header{position:relative;border:1px solid var(--edge);border-radius:28px;padding:24px;background:
      linear-gradient(180deg, rgba(13,64,38,.88), rgba(8,40,22,.88));
      box-shadow:0 10px 40px rgba(0,0,0,.35);overflow:hidden;margin-bottom:22px}
    .flag{width:120px;height:72px;border-radius:6px;border:1px solid #0d4126;background:var(--green);position:relative;box-shadow:0 4px 20px #0005 inset;margin-bottom:12px}
    .flag::after{ /* سيف أبيض رمزي */
      content:""; position:absolute; bottom:14px; left:14px; right:14px; height:6px; background:#fff; border-radius:4px;
      box-shadow:0 0 0 1px rgba(0,0,0,.08) inset;
    }
    .ribbon{position:absolute;inset-inline-start:-60px;top:-60px;width:220px;height:220px;background:
      radial-gradient(circle at 70% 70%, #16c172, #0b6f3d);transform:rotate(35deg);filter:blur(18px);opacity:.18;pointer-events:none}
    h1{margin:6px 0 0 0;font-size:34px}
    ...
    .card{position:relative;background:linear-gradient(180deg,rgba(8,40,22,.95),rgba(8,40,22,.86));
      border:1px solid var(--edge);border-radius:20px;padding:18px 16px 14px;overflow:hidden;transition:transform .2s ease, box-shadow .2s ease}
    .card:before{content:"";position:absolute;inset:-1px;border-radius:22px;padding:1px;background:
      linear-gradient(90deg, #0b6f3d, #14c38e, #0b6f3d);-webkit-mask:linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0);
    ...
    .pill{display:inline-flex;gap:6px;align-items:center;background:#0c3b23;color:#c6f5df;border:1px solid #135b36;border-radius:999px;padding:4px 10px;font-size:12px}
    .title{font-size:20px;margin:10px 0 4px 0}
    .meta{color:#bfe8d6;font-size:13px;margin-bottom:6px}
    .story{white-space:pre-line}
    .sources{margin-top:10px;border-top:1px dashed #1a5b39;padding-top:8px}
    .sources h4{margin:0 0 6px 0;font-size:13px;color:#bfe8d6}
    .sources ul{margin:0;padding:0 18px}
    .sources li{margin:4px 0}
    section h2.section{font-size:24px;margin:22px 0 10px 0}
    .poem{background:linear-gradient(180deg,#0b2b18,#0a2515);border:1px solid #134d30;border-radius:18px;padding:16px}
    footer{margin-top:28px;color:#cbead9;font-size:12px;opacity:.9}
  </style>
</head>
<body>
  <div class="container">
...
قصة: يبرز البيت الجانب الأخلاقي في حكم الملك عبدالعزيز؛ فالنهى تعني الحكمة والإرشاد، والعدل أساس حكمه. يقرّر أنّ سلطته مورِسَت بلا ظلمٍ أو استهتار، بما يبعث الثقة في عدالة الحكم ورشد الإدارة.
    ...
    <section aria-label="المصادر" id="all-sources">
      ...
      <ul>
        ...
        <li><a target="_blank" rel="noopener" href="https://www.vision2030.gov.sa/en/overview">رؤية السعودية 2030 — Overview</a></li>
        <li><a target="_blank" rel="noopener" href="https://ngha.med.sa/english/MediaCenter/News/Pages/rana-reem-twins-separation-48-hours.aspx">الشؤون الصحية بالحرس الوطني — متابعة نجاح فصل «رنا وريـم»</a></li>
